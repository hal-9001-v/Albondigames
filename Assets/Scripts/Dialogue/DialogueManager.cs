...
public class DialogueManager : MonoBehaviour
{
    ...
    public GameObject answerCanvasGroupObject;
    private CanvasGroup answerCanvasGroup;
    ...
    private void Awake()
    {
        ...
        answerCanvasGroup = answerCanvasGroupObject.GetComponent<CanvasGroup>();

        text = bodyObject.GetComponent<TMP_Text>();
        ...
        trigger = GetComponent<LogicTrigger>();
    ...
    private void startSimpleDialogue(Dialogue dialogue)
    {
        ...
        if (!busy)
        {
            ...
            if (dialogue.dialogueName.Equals("Baco Gyal")) SoundManager.PlaySound(SoundManager.Sound.gyalTalks, 0.5f);
            if (dialogue.dialogueName.Equals("Pastafari")) SoundManager.PlaySound(SoundManager.Sound.pastaTalks, 0.5f);
            if (dialogue.dialogueName.Equals("Ra")) SoundManager.PlaySound(SoundManager.Sound.raBallDies, 0.5f);
            if (dialogue.dialogueName.Equals("Loki")) SoundManager.PlaySound(SoundManager.Sound.lokiTalks, 0.5f);
            if (dialogue.dialogueName.Equals("Maricarmen")) SoundManager.PlaySound(SoundManager.Sound.mariTalks, 0.5f);
            if (dialogue.dialogueName.Equals("Nórdico")) SoundManager.PlaySound(SoundManager.Sound.nordTalks, 0.5f);
            if (dialogue.dialogueName.Equals("Isis")) SoundManager.PlaySound(SoundManager.Sound.isisTalks,0.5f);
            if (dialogue.dialogueName.Equals("Anubis")) SoundManager.PlaySound(SoundManager.Sound.isisTalks,0.5f);
            if (dialogue.dialogueName.Equals("Fenrir")) SoundManager.PlaySound(SoundManager.Sound.fenrirOdinTalks, 0.5f);
            if (dialogue.dialogueName.Equals("Odín")) SoundManager.PlaySound(SoundManager.Sound.fenrirOdinTalks, 0.5f);
            if (dialogue.dialogueName.Equals("Zeus")) SoundManager.PlaySound(SoundManager.Sound.zeusTalks, 0.5f);
            if (dialogue.dialogueName.Equals("Afrodeeta")) SoundManager.PlaySound(SoundManager.Sound.zeusTalks, 0.5f);
            simpleDialogue = true;

            //Debug.Log("New Dialogue with: " + dialogue.dialogueName);
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            {
                trigger.triggerThisDialogue();
                errorDialogueDisplayed = true;
            }
        }
